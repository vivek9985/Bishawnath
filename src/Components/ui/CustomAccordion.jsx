        ...
        <Item
            ...
            header={({ state: { isEnter } }) => (
                <>
                    ...
                    <div className={`w-6 md:w-8 h-6 md:h-8 rounded-full text-2xl bg-black dark:bg-primary flex items-center justify-center md:text-3xl xl:text-4xl absolute right-5 top-1/2 -translate-y-1/2 transition-transform duration-300 ease-out ${isEnter && "rotate-[135deg]"}`}>
                        <FaPlus className="text-white dark:text-black text-sm md:text-xl" />
                    </div>
                </>
            )}
            className={({ isEnter }) => `rounded-3xl border-[3px] border-[#f8f8f8] dark:border-[#1d1d1d] bg-white dark:bg-black/10 outfit transition-all duration-500 ${isEnter ? "" : ""}`}
            buttonProps={{
